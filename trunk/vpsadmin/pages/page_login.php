...
if ($_GET[action] == 'login') {
	if (($_REQUEST[passwd] != '') && ($_REQUEST[username])) {
		$sql = 'SELECT * FROM members WHERE m_pass = "'.$db->check(md5($_REQUEST[username].$_REQUEST[passwd])).'" AND m_nick = "'.$db->check($_REQUEST[username]).'"';
...
if ($_GET[action] == 'logout') {
	...
	$xtpl->delayed_redirect('?page=', 2000);
	session_destroy();
}
